...
public class StatPrinter {
    ...
    { 
	_frequency=new ArrayList<Integer> ();//frequency array
	int counter=0;
	for(int b=0;b<=max(data);b++) {//find the highest value of data, for each value between 0, and max(data), count number of instances of each number in list
	    counter=0;
	    for(int i=0; i<data.size(); i++) {
		if (data.get(i)==b)//if there is an element aht is equal to the one we are looking for
		    counter+=1; //add one to the instance counter
	    }
	    _frequency.add(counter);//add to list after parsing through all of data
	}
	...
	for(int i=0;i<data.size();i++) {
	    if (data.get(i)>max) {
		max=data.get(i);//reigning champ method of finding max. GOes through whole list, then finds the absolute maximum of all terms in list
	    }
	}
	return max;
    }
    ...
    {
	boolean f=false;
	int b=_frequency.get(i);
        if(i>=1 &&i<_frequency.size()-1) {//if element isn't first or last element
	    if(_frequency.get(i-1)<b && _frequency.get(i+1)<b) {//and is bigger than the two adjacent ones
		f=true;//it is a local mode
	    }
	}
	return f;
    }
	...
	for(int i=0;i<_frequency.size();i++) {
	    if(isLocalMode(i)) {
		n.add(i);//adds number if its a local mode
	    }
	}
	return n;
    ...
    {
	double proportion;//stores how many asteriks we need
        System.out.println("histogram:\n");
	for(int i=0;i<_frequency.size(); i++) {//goes through each number between 0 and max num in freq list
	    
	    System.out.print(i+":");
	    proportion=((double)_frequency.get(i)/max(_frequency))*longestBar;//longestBar is 
	    for(int x=(int)proportion;x>0;x--) {
		System.out.print("*");
	    }
	    System.out.println("\n");
	}
